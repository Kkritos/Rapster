        ...
        while i<N_BBH+1:
            ...
            while N_BBH>0:
                ...
                if type_int==1: # BBH-star interaction occurs: a field star is gravitationally
                                 ...
                                 # First check whether the encounter results in tidal
                                 # disruption of the star; if not, fall through to treat
                                 # the star as a generic third body in 3-body dynamics below.
                    ...
                    if r_p_tde > r_t_bin:
                        ...
                        # no counter increment -- this is not a TDE. Falls through below to be
                        # treated as a generic 3-body encounter (m3 = m_avg).
                        pass
                        ...
                        # whole-binary/Channel-A disruption event); beta = r_t_bin/r_p_tde is the
                        # penetration parameter, angles set to 0 since this isn't a single-BH
                        # marginally-bound-orbit calculation:
                        tdes = np.append(tdes, [[seed, t, z, 22, m_star, R_star, m1, s1, g1, r_t_bin, r_p_tde, r_t_bin/r_p_tde, 0.0, 0.0, dm1, s1_new, v_rel_tde, h1]], axis=0)
                        tdes = np.append(tdes, [[seed, t, z, 22, m_star, R_star, m2, s2, g2, r_t_bin, r_p_tde, r_t_bin/r_p_tde, 0.0, 0.0, dm2, s2_new, v_rel_tde, h2]], axis=0)
                        ...
                        # The star's pericenter is small enough (r_p_tde <= a) that it penetrates
                        ...
                        # Channel-B disruption event), beta = r_t/r_p_tde is the penetration parameter:
                        tdes = np.append(tdes, [[seed, t, z, 21, m_star, R_star, m_d, s_d, g_d, r_t, r_p_tde, r_t/r_p_tde, 0.0, 0.0, dm, s_new, v_rel_tde, h_d]], axis=0)
                    ...
                    if disrupted:
                        # star was consumed by the disruption -- it cannot also act as a
