...
In addition, ``Data/x_mb_vec.pkl`` is a precomputed lookup table generated by ``Data/.marginally_bound.py`` for the marginally bound radius normalized to the gravitational radius in the dimensionless spin-inclination parameter space grid.

Finally, ``Data/APR.txt`` and ``Data/AU.txt`` contain lookup tables for the APR and AU equations of state for NSs, radius (in km) as a function of NS mass (Msun).
...
- [K. Kritos, D. Wadekar, E. Berti (2026, to appear)]
